...
class NotionExporter:
    ...
    async def export(self, data: Dict, output_path: str):
        ...
        notion_data = {"version": "1.0", "generated_at": datetime.utcnow().isoformat(), "databases": {}}
    ...
    def export_repos(self, github_data: Dict) -> Dict:
        ...
        repos_db = {"schema": self.database_schemas["repos"], "entries": []}
        ...
        for repo_name, repo_data in github_data.get("repos", {}).items():
            ...
            entry = {
                "properties": {
                    ...
                    "Path": {"rich_text": [{"text": {"content": repo_data.get("path", "")}}]},
                    "Remote URL": {"url": repo_data.get("remote_url", "")},
                    "Total Commits": {"number": repo_data.get("total_commits", 0)},
    ...
    def export_ucf_metrics(self, ucf_data: Dict) -> Dict:
        ...
        ucf_db = {"schema": self.database_schemas["ucf_metrics"], "entries": []}
        ...
        entry = {
            "properties": {
                ...
                "Drishti": {"number": ucf_data.get("drishti", 0)},
                "Klesha": {"number": ucf_data.get("klesha", 0)},
                "Zoom": {"number": ucf_data.get("zoom", 0)},
                "Collective Emotion": {"select": {"name": ucf_data.get("collective_emotion", "Unknown")}},
                "Ethical Alignment": {"number": ucf_data.get("ethical_alignment", 0)},
            }
        }
    ...
    def export_agents(self, agent_data: Dict) -> Dict:
        ...
        agents_db = {"schema": self.database_schemas["agents"], "entries": []}
        ...
        for agent in agents:
            entry = {
                "properties": {
                    "Name": {"title": [{"text": {"content": agent.get("name", "Unknown")}}]},
                    "Role": {"rich_text": [{"text": {"content": agent.get("role", "N/A")}}]},
                    "Status": {"select": {"name": agent.get("status", "Unknown")}},
                    "Tasks Completed": {"number": agent.get("tasks_completed", 0)},
                    "Success Rate": {"number": agent.get("success_rate", 0)},
                    "Last Active": {"date": {"start": agent.get("last_active", datetime.utcnow().isoformat())}},
                }
            }
