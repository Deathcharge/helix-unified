...
def get_stripe_price_id(tier: str, billing_cycle: str) -> str:
    ...
    if not price_id:
        raise HTTPException(status_code=400, detail=f"Invalid tier '{tier}' or billing cycle '{billing_cycle}'")
...
def get_tier_limits(tier: str) -> Dict[str, int]:
    ...
    """
    limits = {
        "free": {"requests_per_day": 100, "agents_allowed": 3, "prompts_allowed": 10, "workflows_allowed": 0},
        "pro": {"requests_per_day": 10000, "agents_allowed": 14, "prompts_allowed": -1, "workflows_allowed": 10},  # Unlimited
        "workflow": {"requests_per_day": 20000, "agents_allowed": 14, "prompts_allowed": -1, "workflows_allowed": 100},
        "enterprise": {
...
async def get_or_create_stripe_customer(user_id: str, email: str) -> str:
    ...
    try:
        ...
        # Store customer ID
        await Database.execute("UPDATE users SET stripe_customer_id = $1 WHERE id = $2", customer.id, user_id)
...
async def create_subscription(
    user_id: str, email: str, tier: str, billing_cycle: str = "monthly", payment_method_id: Optional[str] = None
...
async def cancel_subscription(user_id: str, immediate: bool = False) -> Dict[str, Any]:
    ...
    """
    # Get subscription ID
    subscription_id = await Database.fetchval("SELECT stripe_subscription_id FROM users WHERE id = $1", user_id)
    ...
    try:
        if immediate:
            ...
            await Database.execute("UPDATE users SET subscription_status = 'canceling' WHERE id = $1", user_id)

            return {
                "status": "canceling",
                "cancel_at": datetime.fromtimestamp(subscription.current_period_end),
                "message": f"Subscription will be canceled on {datetime.fromtimestamp(subscription.current_period_end).strftime('%Y-%m-%d')}",  # noqa
            }
...
async def update_subscription(
    user_id: str, new_tier: Optional[str] = None, new_billing_cycle: Optional[str] = None
) -> SubscriptionResponse:
    """
    ...
    row = await Database.fetchrow("SELECT stripe_subscription_id, tier FROM users WHERE id = $1", user_id)
...
async def create_checkout_session(
    user_id: str, email: str, tier: str, billing_cycle: str, success_url: str, cancel_url: str
) -> Dict[str, str]:
    """
...
async def handle_subscription_updated(subscription: Dict[str, Any]):
    ...
    if not user_id:
        ...
        user_id = await Database.fetchval("SELECT id FROM users WHERE stripe_subscription_id = $1", subscription["id"])
...
async def handle_payment_succeeded(invoice: Dict[str, Any]):
    ...
    # Find user
    user_id = await Database.fetchval("SELECT id FROM users WHERE stripe_customer_id = $1", customer_id)
    ...
    await Database.execute("UPDATE users SET subscription_status = 'active' WHERE id = $1", user_id)
...
async def handle_payment_failed(invoice: Dict[str, Any]):
    ...
    await Database.execute("UPDATE users SET subscription_status = 'past_due' WHERE id = $1", user_id)
