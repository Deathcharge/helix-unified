...
async def post_daily_trends():
    ...
    try:
        ...
        embed.add_field(
            name="🌀 Harmony",
            value=f"`{ucf_state.get('harmony', 0):.4f}`",
            inline=True
        )
        embed.add_field(
            name="🛡️ Resilience",
            value=f"`{ucf_state.get('resilience', 0):.4f}`",
            inline=True
        )
        embed.add_field(
            name="🔥 Prana",
            value=f"`{ucf_state.get('prana', 0):.4f}`",
            inline=True
        )
        embed.add_field(
            name="👁️ Drishti",
            value=f"`{ucf_state.get('drishti', 0):.4f}`",
            inline=True
        )
        embed.add_field(
            name="🌊 Klesha",
            value=f"`{ucf_state.get('klesha', 0):.4f}`",
            inline=True
        )
        embed.add_field(
            name="🔍 Zoom",
            value=f"`{ucf_state.get('zoom', 0):.4f}`",
            inline=True
        )
        ...
        embed.add_field(
            name="📈 Progress to v15.3 Goal",
            value=f"`{progress:.1f}%` (Target: 0.60)",
            inline=False
        )
        ...
        if storage_mode in ["nextcloud", "mega"]:
            try:
                ...
                await upload_samsara_asset(chart_path, {
