...
class PortalDeploymentCommands(commands.Cog):
    ...
    def _parse_portal_command(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        if 'status' in text or 'check' in text:
            return {
                'action': 'status',
                'message': 'Checking portal deployment status'
            }
    ...
    async def deploy(self, ctx: commands.Context, *, target: str = "help"):
        ...
        if not command:
            await ctx.send(
                "❓ I didn't understand that command. Use `!deploy help` for available commands."
            )
            return
        ...
        if isinstance(self.portals, dict):
            for category, portals in self.portals.items():
                ...
                if isinstance(portals, list) and portals:
                    portal_names = [p.get('name', p.get('id', 'Unknown')) if isinstance(p, dict) else str(p) for p in portals[:5]]
                    count = len(portals)
                    
                    embed.add_field(
                        name=f"{category.title()} Portals ({count})",
                        value="\n".join([f"• {name}" for name in portal_names]) + (f"\n... and {count - 5} more" if count > 5 else ""),
                        inline=False
                    )
        
        embed.add_field(
            name="Total Portals",
            value=f"**{self.portals.get('total', 51)}** portals in constellation",
            inline=False
        )
